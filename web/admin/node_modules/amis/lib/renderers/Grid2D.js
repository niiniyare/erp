...
var Grid2D = /** @class */function (_super) {
  ...
  Grid2D.prototype.renderGrid = function (grid, key, length) {
    ...
    }, itemRender ? itemRender(grid, key, length, this.props) : this.renderChild("grid2d/".concat(key), grid));
  };
  Grid2D.prototype.renderGrids = function () {
    var _this = this;
    var grids = this.props.grids;
    return grids.map(function (grid, key) {
      return _this.renderGrid(grid, key, grids.length);
    });
  };
  Grid2D.prototype.render = function () {
    ...
    grids.forEach(function (grid) {
      if (grid.width) {
        templateColumns[grid.x - 1] = Number.isInteger(grid.width) ? grid.width + 'px' : grid.width;
      }
      if (grid.height) {
        templateRows[grid.y - 1] = Number.isInteger(grid.height) ? grid.height + 'px' : grid.height;
      }
    });
    var curStyle = tslib.__assign(tslib.__assign({}, style), {
      display: 'grid',
      columnGap: gap,
...
var Grid2DRenderer = /** @class */function (_super) {
  ...
  Grid2DRenderer = tslib.__decorate([amisCore.Renderer({
    type: 'grid-2d'
  })], Grid2DRenderer);
