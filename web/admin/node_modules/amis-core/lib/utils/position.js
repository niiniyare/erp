...
function position(node, offsetParent$1) {
    ...
    if (getComputedStyle(node).getPropertyValue('position') === 'fixed') {
        ...
        var parent_1 = offsetParent$1 || offsetParent.offsetParent(node);
        offset$1 = offset.offset(node);
        if (parent_1 === node) {
            return {
                top: 0,
                left: 0,
        ...
        if (nodeName(parent_1) !== 'html')
            parentOffset = offset.offset(parent_1);
        var parentStyle = getComputedStyle(parent_1);
        var borderTop = String(parentStyle.getPropertyValue('border-top-width') || 0);
        parentOffset.top += parseInt(borderTop, 10) - parent_1.scrollTop || 0;
        var borderLeft = String(parentStyle.getPropertyValue('border-left-width') || 0);
