...
function isObjectShallowModified(prev, next, strictModeOrFunc, ignoreUndefined, stack, maxDepth) {
    ...
    if (isNaN__default["default"](prev) && isNaN__default["default"](next)) {
        return false;
    }
    if (null == prev ||
        null == next ||
...
function isVisible(schema, data, statusStore) {
    ...
    if ((schema.id || schema.name) && statusStore) {
        var id = tpl.filter(schema.id, data);
        var name_1 = tpl.filter(schema.name, data);
...
function isEmpty(thing) {
    return !(object.isObject(thing) && Object.keys(thing).length);
}
/**
 * 基于时间戳的 uuid
 *
...
function findTree(tree, iterator, withCache) {
    var isValidateKey = function (value) {
        return value !== '' && (isString__default["default"](value) || isNumber__default["default"](value));
    };
    // 缓存优化
...
function findTreeIndex(tree, iterator, withCache) {
    ...
    findTree(tree, function (item, index, level, paths) {
        ...
        ? undefined
        : tslib.__assign(tslib.__assign({}, withCache), { foundEffect: foundEffect }));
...
function spliceTree(tree, idx, deleteCount) {
    ...
    if (typeof idx === 'number') {
        list.splice.apply(list, tslib.__spreadArray([idx, deleteCount], tslib.__read(items), false));
    }
        ...
        host.splice.apply(host, tslib.__spreadArray([lastIdx, deleteCount], tslib.__read(items), false));
    }
    return list;
}
/**
        ...
        if (Array.isArray(options.columns)) {
            options.columns.forEach(function (column) {
                var value = typeof column.name === 'string'
                    ? getVariable.getVariable(options.query, column.name)
                    : undefined;
                var key = column.name;
                if ((options.filterOnAllColumns ||
                    ...
                    if (Array.isArray(value)) {
                        if (value.length > 0) {
                            var arr_1 = tslib.__spreadArray([], tslib.__read(items), false);
                            var arrItems_1 = [];
                        ...
                        items = matchSorter.matchSorter(items, value, {
                            keys: [key],
                            threshold: matchSorter.matchSorter.rankings.CONTAINS
                        });
                    }
    ...
    return function () {
        ...
        return fns.reduce(function (ret, fn) {
            return ret === false
                ? false
                : typeof fn == 'function'
                    ? fn.apply(void 0, tslib.__spreadArray([], tslib.__read(args), false)) : undefined;
        }, undefined);
    };
}
...
var ValidateError = /** @class */ (function (_super) {
    tslib.__extends(ValidateError, _super);
...
function resolveValueByName(data, name, canAccessSuper) {
    return isPureVariable.isPureVariable(name)
        ? resolveVariableAndFilter.resolveVariableAndFilter(name, data, '|raw')
...
function getPropValue(props, getter, canAccessSuper) {
    ...
    return ((_b = (_a = value !== null && value !== void 0 ? value : getter === null || getter === void 0 ? void 0 : getter(props)) !== null && _a !== void 0 ? _a : resolveValueByName(data, name, canAccessSuper)) !== null && _b !== void 0 ? _b : (formula.isExpression(defaultValue)
        ? resolveVariableAndFilter.resolveVariableAndFilter(defaultValue, data)
        : formula.replaceExpression(defaultValue)));
}
...
function JSONValueMap(json, mapper, deepFirst, stack) {
    ...
    if (stack === void 0) { stack = []; }
    if (!isPlainObject__default["default"](json) && !Array.isArray(json)) {
        return json;
    }
    ...
    return modified
        ? tslib.__assign(tslib.__assign({}, json), toUpdate) : json;
}
...
function buildTrackExpression(schema) {
    if (!isPlainObject__default["default"](schema) && !Array.isArray(schema)) {
        return '';
    }
    ...
    JSONTraverse(schema, function (value, key) {
        ...
        if (key === 'name') {
            trackExpressions.push(isPureVariable.isPureVariable(value) ? value : "${".concat(value, "}"));
        }
        else if (key === 'source') {
...
function evalTrackExpression(expression, data) {
    ...
    return parts
        .map(function (item) {
        if (item.type === 'text') {
            return tokenize.tokenize(item.value, data);
        }
        return tpl.evalExpression(item.value, data);
    })
        .join('');
}
...
var TestIdBuilder = /** @class */ (function () {
    ...
    TestIdBuilder.prototype.getChild = function (childPath, data) {
        ...
        return new TestIdBuilder(data
            ? tpl.filter("".concat(this.testId, "-").concat(childPath), data)
    ...
    TestIdBuilder.prototype.getTestId = function (data) {
        ...
        return _a = {},
            _a[TEST_ID_KEY] = data ? tpl.filter(this.testId, data) : this.testId,
            _a;
    };
    TestIdBuilder.prototype.getTestIdValue = function (data) {
        ...
        return data ? tpl.filter(this.testId, data) : this.testId;
    };
...
function formateCheckThemeCss(themeCss, type) {
    ...
    return tslib.__assign(tslib.__assign({}, themeCss), (_a = {}, _a["".concat(type, "ControlClassName")] = defaultControlThemeCss, _a["".concat(type, "ControlCheckedClassName")] = checkedControlThemeCss, _a["".concat(type, "ClassName")] = defaultThemeCss, _a["".concat(type, "CheckedClassName")] = checkedThemeCss, _a));
}
...
exports.changedEffect = changedEffect;
...
exports.differenceFromAll = differenceFromAll;
exports.eachTree = eachTree;
exports.evalTrackExpression = evalTrackExpression;
...
exports.formateCheckThemeCss = formateCheckThemeCss;
...
exports.importLazyComponent = importLazyComponent;
...
exports.parsePrimitiveQueryString = parsePrimitiveQueryString;
