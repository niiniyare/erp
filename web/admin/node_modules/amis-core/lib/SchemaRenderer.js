...
var SchemaRenderer = /** @class */ (function (_super) {
    ...
    SchemaRenderer.prototype.handleGlobalVarChange = function () {
        ...
        var chain = object.extractObjectChain(this.props.data).filter(function (item) { return !item.hasOwnProperty('__isTempGlobalLayer'); });
        var globalLayerIdx = findLastIndex__default["default"](chain, function (item) {
        ...
        var newData = object.createObjectFromChain(chain);
    ...
    SchemaRenderer.prototype.renderChild = function (region, node, subProps) {
        ...
        var _e = this.props, _ = _e.schema; _e.$path; var env = _e.env, render = _e.render, rest = tslib.__rest(_e, ["schema", "$path", "env", "render"]);
    ...
    SchemaRenderer.prototype.render = function () {
        ...
        // style 支持公式
        if (schema.style) {
            props.style = style.mergeStyle(props.style, style.buildStyle(schema.style, detectData));
        }
        ...
        if (rest.env.enableTestid) {
            if (props.testid || props.id || props.testIdBuilder == null) {
                if (!(props.testIdBuilder instanceof helper.TestIdBuilder)) {
                    props.testIdBuilder = new helper.TestIdBuilder(props.testid || props.id);
                }
            }
        }
        ...
        var component = supportRef ? (_J$X_(Component, tslib.__assign({}, props, { ref: this.childRef, storeRef: this.storeRef }))) : (_J$X_(Component, tslib.__assign({}, props, { forwardedRef: this.childRef, storeRef: this.storeRef })));
        if (schema.animations) {
