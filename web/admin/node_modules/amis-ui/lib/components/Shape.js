...
var vectorComputed = require('../utils/vectorComputed.js');
...
var SvgPathGenerator = /** @class */ (function () {
    ...
    SvgPathGenerator.prototype.radiusPath = function (radius, _a, _b, _c) {
        var _d = tslib.__read(_a, 2), endX1 = _d[0], endY1 = _d[1];
        var _e = tslib.__read(_b, 2), startX = _e[0], startY = _e[1];
        var _f = tslib.__read(_c, 2), endX2 = _f[0], endY2 = _f[1];
        var flag = true;
        if (radius < 0) {
            radius = radius * -1;
            // 圆角向内
        ...
        var _g = vectorComputed.radiusStartEndPoint([startX, startY, endX1, endY1], [startX, startY, endX2, endY2], radius), start = _g.start, end = _g.end, mxRadius = _g.radius, sweepFlag = _g.sweepFlag;
        return [
    ...
    SvgPathGenerator.prototype.addRoundCornersToPath = function (points, radius) {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            var _a = tslib.__read(points[i], 2), startX = _a[0], startY = _a[1];
            var _b = tslib.__read(nextItem, 2), endX1 = _b[0], endY1 = _b[1];
            var _c = tslib.__read(preItem, 2), endX2 = _c[0], endY2 = _c[1];
            if (!radius) {
                newPath.push("".concat(i === 0 ? 'M' : 'L').concat(startX, ",").concat(startY));
                continue;
            }
            var _d = tslib.__read(this.radiusPath(radius, [endX2, endY2], [startX, startY], [endX1, endY1]), 2), start = _d[0], A = _d[1];
...
var Shape = function (props) {
    ...
    var getStyle = React__default["default"].useCallback(function () { return [
        {
            width: width + 'px',
            height: height + 'px'
        },
    ...
    return (_J$X_("div", { className: cx('Shape', className), style: getStyle()[0], onClick: props.onClick },
        _J$X_("svg", { className: cx('Shape-svg'), width: BASE_SIZE, height: BASE_SIZE, style: getStyle()[1], fill: color ? color : 'currentColor', viewBox: "0 0 ".concat(BASE_SIZE, " ").concat(BASE_SIZE) }, paths.map(function (path, index) { return (_J$X_("path", tslib.__assign({}, strokeProps, { key: index, d: path }))); }))));
};
var Shape$1 = amisCore.themeable(Shape);

exports["default"] = Shape$1;
