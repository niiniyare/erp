...
var CityArea = React.memo(function (props) {
    var _a;
    ...
    _h = props.value, 
    // 默认北京东城区
    value = _h === void 0 ? 110101 : _h, cx = props.classnames, style = props.style, __ = props.translate, _j = props.disabled, disabled = _j === void 0 ? false : _j, popOverContainer = props.popOverContainer, mobileUI = props.mobileUI;
    var _k = tslib.__read(React.useState([]), 2), values = _k[0], setValues = _k[1];
    var _l = tslib.__read(React.useState(''), 2), street = _l[0], setStreet = _l[1];
    var _m = tslib.__read(React.useState(), 2), confirmValues = _m[0], setConfirmValues = _m[1];
    var _o = tslib.__read(useSetState["default"](), 2), db = _o[0], updateDb = _o[1];
    var _p = tslib.__read(useSetState["default"]({
        columns: []
    }), 2), state = _p[0], updateState = _p[1];
    var _q = tslib.__read(React.useState(false), 2), isOpened = _q[0], setIsOpened = _q[1];
    var onChange = function (columnValues, columnIndex) {
        ...
        var _f = tslib.__read(columnValues, 3), provience = _f[0], city = _f[1], district = _f[2];
        if (city === -1) {
    ...
    var propsChange = function () {
        ...
        var _a = tslib.__read(values, 3), province = _a[0], city = _a[1], district = _a[2];
    ...
    var updateColumns = function () {
        ...
        var _a = tslib.__read(values, 3), provience = _a[0], city = _a[1], district = _a[2];
    ...
    var loadDb = function () {
        Promise.resolve().then(function() {return new Promise(function(fullfill) {require(['./CityDB.js', "tslib"], function(mod, tslib) {fullfill(tslib.__importStar(mod))})})}).then(function (db) {
            updateDb(tslib.__assign(tslib.__assign({}, db.default), { province: db.province, city: db.city, district: db.district }));
        });
    };
    ...
    return (_J$X_("div", { className: cx("CityArea"), style: style },
        _J$X_(ResultBox["default"], { className: cx('CityArea-Input', isOpened ? 'is-active' : ''), allowInput: false, result: result, onResultChange: function () { }, onResultClick: function () { return setIsOpened(!isOpened); }, placeholder: __('Select.placeholder'), mobileUI: mobileUI, disabled: disabled }),
        allowStreet && values[0] ? (_J$X_("input", { className: cx('CityArea-Input'), value: street, onChange: function (e) {
                return setStreet(e.currentTarget.value);
            }, placeholder: __('City.street'), disabled: disabled, readOnly: true })) : null,
        _J$X_(PopUp["default"], { className: cx("CityArea-popup"), container: popOverContainer, isShow: isOpened, showConfirm: true, onConfirm: onConfirm, onHide: onCancel },
            _J$X_(Picker["default"], { className: 'CityArea-picker', columns: state.columns, onChange: onChange, showToolbar: false, labelField: "text", itemHeight: 40, value: values, classnames: props.classnames, classPrefix: props.classPrefix }))));
});
var CityArea$1 = amisCore.themeable(amisCore.localeable(amisCore.uncontrollable(CityArea, {
    value: 'onChange'
})));

exports["default"] = CityArea$1;
