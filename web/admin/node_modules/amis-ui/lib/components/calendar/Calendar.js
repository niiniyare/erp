...
var CalendarContainer = require('./CalendarContainer.js');
...
var BaseDatePicker = /** @class */ (function (_super) {
    tslib.__extends(BaseDatePicker, _super);
    function BaseDatePicker(props) {
        ...
        _this.setTime = function (type, value) {
            ...
            if (!_this.props.value) {
                _this.setState({
                    selectedDate: date,
                    inputValue: date.format(state.displayForamt)
                });
            }
            _this.props.onChange && _this.props.onChange(date, 'time');
        };
        _this.setDate = function (type) {
            // todo 没看懂这个是啥意思，好像没啥用
            var currentShould = _this.props.viewMode === 'months' &&
                !/^mm$/i.test((_this.props.inputFormat || _this.props.displayForamt) || '');
            var nextViews = {
        ...
        _this.updateSelectedDate = function (e, close) {
            ...
            if (!_this.props.value) {
                ...
                _this.setState({
                    ...
                    inputValue: date === null || date === void 0 ? void 0 : date.format(_this.state.displayForamt),
                    open: open
                });
            }
            else {
        ...
        _this.onConfirm = function (value, types) {
            var currentDate = (_this.state.selectedDate ||
                ...
                moment__default["default"]()).clone();
            var date = amisCore.convertArrayValueToMoment(value, types, currentDate);
            if ((types === null || types === void 0 ? void 0 : types[1]) === 'quarter') {
            ...
            if (!_this.props.value) {
                _this.setState({
                    selectedDate: date,
                    viewDate: date,
                    inputValue: date.format(_this.state.displayForamt)
                });
            }
        ...
        _this.onClose = function () {
            _this.setState({ currentView: _this.state.initView });
    ...
    BaseDatePicker.prototype.componentDidUpdate = function (prevProps) {
        ...
        if (props.viewDate !== prevProps.viewDate) {
            updatedState.viewDate = moment__default["default"](props.viewDate);
        }
    ...
    BaseDatePicker.prototype.checkTZ = function (props) {
        var con = console;
        // @ts-ignore
        if (props.displayTimeZone && !this.tzWarning && !moment__default["default"].tz) {
            this.tzWarning = true;
            con &&
    ...
    BaseDatePicker.prototype.localMoment = function (date, format, props) {
        ...
        var m = null;
        if (props.utc) {
            m = moment__default["default"].utc(date, format, props.strictParsing);
        }
        else if (props.displayTimeZone) {
            // @ts-ignore 以后再修
            m = moment__default["default"].tz(date, format, props.displayTimeZone);
        }
        else {
            m = moment__default["default"](date, format, props.strictParsing);
        }
        if (props.locale)
    ...
    BaseDatePicker.prototype.render = function () {
        var _a;
        var _b = this.props, viewMode = _b.viewMode, timeFormat = _b.timeFormat, dateFormat = _b.dateFormat, timeRangeHeader = _b.timeRangeHeader, mobileUI = _b.mobileUI, testIdBuilder = _b.testIdBuilder;
        var Component = CalendarContainer["default"];
        var viewProps = this.getComponentProps();
        if (viewMode === 'quarters') {
            _a = tslib.__read([
                'quarters',
                this.props.renderQuarter
            ], 2), viewProps.updateOn = _a[0], viewProps.renderQuarter = _a[1];
        }
        ...
        return (_J$X_("div", tslib.__assign({ className: cx__default["default"]('rdt rdtStatic rdtOpen', this.props.className, (timeFormat && !dateFormat) || typeof dateFormat !== 'string'
                ? 'rdtTimeWithoutD'
            ...
            _J$X_("div", { key: "dt", className: cx__default["default"]('rdtPicker', {
                    'is-mobile-year': mobileUI && viewMode === 'years'
                }, { 'is-mobile-embed': mobileUI && viewProps.embed }, timeFormat && !dateFormat
                    ...
                    : timeFormat && dateFormat
                        ...
                        : dateFormat && !timeFormat
                            ? 'rdtPickerDate'
                            : '') },
                _J$X_(Component, { view: this.state.currentView, viewProps: viewProps, timeRangeHeader: timeRangeHeader }))));
    };
    return BaseDatePicker;
}(React__default["default"].Component));
var Calendar = amisCore.themeable(BaseDatePicker);
