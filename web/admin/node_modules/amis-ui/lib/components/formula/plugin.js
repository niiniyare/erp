...
function editorFactory(dom, cm, props, options) {
    ...
    return cm(dom, tslib.__assign({ value: props.value || '', autofocus: false, mode: props.evalMode ? 'text/formula' : 'text/formula-template', readOnly: props.readOnly ? 'nocursor' : false }, options));
}
...
var FormulaPlugin = /** @class */ (function () {
    function FormulaPlugin(editor, cm) {
        ...
        this.marks = [];
        // this.autoMarkText();
        this.autoMarkText = debounce__default["default"](this.autoMarkText.bind(this), 250, {
            leading: false,
            trailing: true
        });
    ...
    FormulaPlugin.prototype.autoMark = function () {
        ...
        this.marks = [];
        try {
            var ast = amisFormula.parse(value, {
            ...
            traverseAst(ast, function (ast) {
                ...
                if (ast.type === 'func_call') {
                    ...
                    if ((host_1 === null || host_1 === void 0 ? void 0 : host_1.type) === 'variable') {
                        var variable = amisCore.findTree(variables, function (item) { return item.value === host_1.name; });
                        if (variable) {
                            // 先标记顶层对象
                            _this.markText({
                            ...
                            var _loop_1 = function (i, len) {
                                ...
                                if ((item === null || item === void 0 ? void 0 : item.type) === 'identifier') {
                                    var variable_1 = (_b = amisCore.findTree(vars, function (v) { return v.value === path_1 + item.name; })) !== null && _b !== void 0 ? _b : amisCore.findTree(vars, function (v) { return v.value === item.name; } // 兼容不带路径的情况
                                    );
                                    if (variable_1) {
                                        _this.markText({
                    ...
                    // 直接就是变量
                    var variable = amisCore.findTree(variables, function (item) { return item.value === ast.name; });
                    if (variable) {
                        _this.markText({
                            line: ast.start.line - 1,
                            ch: ast.start.column - 1
