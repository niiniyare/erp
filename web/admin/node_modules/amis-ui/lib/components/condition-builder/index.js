...
var QueryBuilder = /** @class */ (function (_super) {
    tslib.__extends(QueryBuilder, _super);
    function QueryBuilder() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.config = tslib.__assign(tslib.__assign({}, config["default"]), _this.props.config);
        _this.lastMoveAt = 0;
        return _this;
    }
    ...
    QueryBuilder.prototype.handleDragOver = function (e) {
        ...
        if (gIdx !== cgIdx) {
            amisCore.animation.capture(container);
    ...
    QueryBuilder.prototype.handleDragDrop = function () {
        ...
        var indexes = amisCore.findTreeIndex([value], function (item) { return item.id === fromId; });
        if (indexes) {
            var origin_1 = amisCore.getTree([value], indexes.concat());
            _a = tslib.__read(amisCore.spliceTree([value], indexes, 1), 1), value = _a[0];
            var indexes2 = amisCore.findTreeIndex([value], function (item) { return item.id === toId; });
            if (indexes2) {
                _b = tslib.__read(amisCore.spliceTree([value], indexes2.concat(toIndex), 0, origin_1), 1), value = _b[0];
                onChange(value);
            }
        }
    };
    ...
    QueryBuilder.prototype.highlightValue = function (value) {
        ...
        return (_J$X_("div", { className: cx('CPGroup-result'), dangerouslySetInnerHTML: html }));
    };
    ...
    QueryBuilder.prototype.render = function () {
        ...
        var resultBoxValue = embed === false
            ? !value ||
                ...
                : value
            : value;
        return (_J$X_(PickerContainer["default"], { classnames: cx, classPrefix: classPrefix, translate: translate, locale: locale, onConfirm: onFinalChange, value: value, size: 'md', popOverContainer: popOverContainer, bodyRender: function (params) { return _this.renderBody(params.onChange, params.value); }, title: title }, function (_a) {
            var onClick = _a.onClick, isOpened = _a.isOpened;
            return (_J$X_(ResultBox["default"], { classnames: cx, classPrefix: classPrefix, translate: translate, locale: locale, className: cx('CBGroup-result', { 'is-active': isOpened }), allowInput: false, clearable: true, result: resultBoxValue, itemRender: _this.highlightValue, onResultChange: amisCore.noop, onClear: _this.handleClear, disabled: disabled, borderMode: 'full', placeholder: placeholder, actions: pickerIcon && (_J$X_("span", { className: cx('CBPicker-trigger'), onClick: onClick }, pickerIcon)), mobileUI: mobileUI, onResultClick: onClick, testIdBuilder: testIdBuilder === null || testIdBuilder === void 0 ? void 0 : testIdBuilder.getChild('result-box') }));
        }));
    };
    tslib.__decorate([
        amisCore.autobind,
...
var ConditionBuilder = amisCore.themeable(amisCore.localeable(amisCore.uncontrollable(QueryBuilder, {
    value: 'onChange'
})));
