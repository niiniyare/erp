...
import { isObjectShallowModified, validations, isObject, themeable } from 'amis-core';
...
var Rating = /** @class */ (function (_super) {
    ...
    function Rating(props) {
        ...
        _this.ratingRef = React__default.createRef();
        _this.starsNode = {};
        _this.state = {
            value: props.value || 0,
            stars: [],
    ...
    Rating.prototype.componentDidUpdate = function (prevProps) {
        ...
        else if (isObjectShallowModified(props.colors, prevProps.colors) ||
    ...
    Rating.prototype.sortKeys = function (map) {
        ...
        return Object.keys(map)
            .filter(function (item) { return validations.isNumeric({}, item); })
    ...
    Rating.prototype.getShowColorAndText = function (value) {
        ...
        if (this.ratingRef.current) {
            var style = getComputedStyle(this.ratingRef.current);
            var defaultColors = [];
            try {
                defaultColors = JSON.parse(JSON.parse(style.getPropertyValue('content')));
            }
            catch (e) {
                console.warn(e);
            }
            ...
            defaultColors.forEach(function (item) {
                obj_1[item.id] = item.value;
            });
            if (isEmpty(colors) && !isEmpty(obj_1)) {
                tempColor = obj_1;
            }
        }
        if (!value)
        ...
        if (tempColor && typeof tempColor !== 'string') {
            var keys = this.sortKeys(tempColor);
            var showKey = keys.filter(function (item) { return Number(item) < value; }).length;
            var showColor = keys[showKey] !== undefined && tempColor[keys[showKey]];
            // 取最大 key 的颜色，避免如下情况：tempColor 只设置了 1-4，value 为 5，导致取不到颜色而无法显示
            var lastColor = keys.length && tempColor[keys[keys.length - 1]];
            this.setState({
    ...
    Rating.prototype.renderStars = function () {
        ...
        return (React__default.createElement("ul", { onMouseLeave: this.mouseLeave }, stars.map(function (star, i) {
            var isThisHalf = half && !halfStar.hidden && halfStar.at === i;
            return (React__default.createElement("li", { ref: _this.saveRef(i), className: cx('Rating-star', charClassName, {
                    'is-half': isThisHalf,
                ...
                isThisHalf && (React__default.createElement("div", { className: cx('Rating-star-half'), style: {
                        color: showColor
                    } }, char)),
                char));
        })));
    ...
    Rating.prototype.renderText = function () {
        ...
        if (!showText)
            return null;
        return (React__default.createElement("span", { className: cx('Rating-text', textClassName, (_a = {},
    ...
    Rating.prototype.render = function () {
        ...
        return (React__default.createElement("div", { className: cx('Rating', className), ref: this.ratingRef }, textPosition === 'left' ? (React__default.createElement(React__default.Fragment, null,
            this.renderText(),
            this.renderStars())) : (React__default.createElement(React__default.Fragment, null,
            this.renderStars(),
            this.renderText()))));
    };
    Rating.defaultProps = {
        ...
        value: 0,
        count: 5,
        char: React__default.createElement(Icon, { icon: "star", className: "icon", iconContent: "Rating-star-icon" }),
        colors: '',
        textPosition: 'right'
    };
    return Rating;
}(React__default.Component));
var Rating$1 = themeable(Rating);

export { Rating, Rating$1 as default };
