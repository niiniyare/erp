...
import { CT_Row_Attributes } from '../../../../openxml/ExcelTypes.js';
import { CT_Cell_Attributes } from '../../../types/worksheet/ICell.js';
...
function parseSheetData(sheetDataNode, sharedStrings) {
    ...
    var cellData = [];
    try {
        for (var _e = __values(sheetDataNode.children || []), _f = _e.next(); !_f.done; _f = _e.next()) {
            var rowNode = _f.value;
            // 这里不自动填充默认值，因为数据量可能很大，后面代码里判断
            var row = autoParse(rowNode, CT_Row_Attributes);
            // 行号，改成从 0 开始
            var r = (row.r || 1) - 1;
            rows[r] = row;
            try {
                for (var _g = (e_2 = void 0, __values(rowNode.children || [])), _h = _g.next(); !_h.done; _h = _g.next()) {
                    var cellNode = _h.value;
                    var cell = autoParse(cellNode, CT_Cell_Attributes);
                    var cellType = cell.t;
                    var styleIndex = cell.s;
                    /**
                     * 列号，改成从 0 开始
                     */
                    var colNumber = decodeAddress(cell.r).col;
                    // 开始解析 cell 里的数据
                    var value = '';
                    var formula = '';
                    try {
                        for (var _j = (e_3 = void 0, __values(cellNode.children || [])), _k = _j.next(); !_k.done; _k = _j.next()) {
                            ...
                            switch (tag) {
                                ...
                                case 'is':
                                    try {
                                        // 这种基本上是 inlineStr
                                        for (var _l = (e_4 = void 0, __values(cellNodeChild.children || [])), _m = _l.next(); !_m.done; _m = _l.next()) {
                                            var isNode = _m.value;
