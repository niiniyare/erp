import { autoWrapText } from './autoWrapText.js';
import { measureTextWithCache } from './measureTextWithCache.js';
import { drawMultiLineText } from './drawMultiLineText.js';
import { drawSingleLineRichText } from './drawSingleLineRichText.js';
import { drawSingleLineText } from './drawSingleLineText.js';
...
function drawTextInCell(excelRender, sheet, ctx, dataProvider, cellInfo, x, y, width, height, indentSize, padding, linkPositionCache) {
    ...
    if (alignment) {
        ...
        if (alignment.textRotation) {
            ...
            if (alignment.textRotation === 255) {
                ...
                var defaultFontSize = measureTextWithCache(ctx, defaultFont, '1');
                // 这个保证会拆分
    ...
    if (wrapText) {
        ...
        if (cellInfo.text) {
            ...
            lines = autoWrapText(ctx, cellInfo.cellData.richText, displayWidth, fontStyle);
        }
        else {
            console.warn('unknown cell data', cellInfo);
        }
        drawMultiLineText(excelRender, sheet, ctx, dataProvider, fontStyle, lines, x + padding, y + padding, displayWidth, height - padding * 2, padding, horizontal, vertical, cellInfo.text, cellInfo.row, linkPositionCache);
    }
    else {
        // 为了提升性能这个单独实现，因为是最常见的情况
        if (cellInfo.text) {
            drawSingleLineText(ctx, fontStyle, cellInfo.text, fontStyle.color, x + padding, y + padding, displayWidth, height - padding * 2, indent * indentSize, horizontal, vertical, linkPositionCache);
        }
        else if (typeof cellInfo.cellData === 'object' &&
            'richText' in cellInfo.cellData) {
            drawSingleLineRichText(excelRender, sheet, ctx, dataProvider, fontStyle, cellInfo.cellData.richText, x + padding, y + padding, displayWidth, height - padding * 2, horizontal, vertical, cellInfo.row);
        }
    }
    if (needClip) {
        ctx.restore();
