...
import { cellInfoToStyle } from './cellInfoToStyle.js';
...
function rangeToHTML(workbook, range) {
    ...
    for (var row = range.startRow; row <= endRow; row++) {
        ...
        for (var col = range.startCol; col <= endCol; col++) {
            ...
            var text = escapeHtml(cellInfo.text);
            var style = cellInfoToStyle(workbook, cellInfo);
            tsvRow.push(text);
            ...
            if (mergeCell.startCol !== mergeCell.endCol &&
                ...
                tsvRow.push.apply(tsvRow, __spreadArray([], __read(Array(colSpan - 1).fill('')), false));
            }
            else {
