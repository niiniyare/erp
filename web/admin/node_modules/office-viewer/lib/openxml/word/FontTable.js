'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var tslib = require('tslib');
var Font = require('./Font.js');

/**
 * 解析和渲染字体
 */
var FontTable = /** @class */ (function () {
    function FontTable() {
        this.fonts = [];
    }
    FontTable.fromXML = function (word, doc) {
        var e_1, _a;
        var fonts = Array.from(doc.getElementsByTagName('w:font'));
        var fontTable = new FontTable();
        try {
            for (var fonts_1 = tslib.__values(fonts), fonts_1_1 = fonts_1.next(); !fonts_1_1.done; fonts_1_1 = fonts_1.next()) {
                var child = fonts_1_1.value;
                var font = Font.Font.fromXML(word, child);
                fontTable.fonts.push(font);
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
        finally {
            try {
                if (fonts_1_1 && !fonts_1_1.done && (_a = fonts_1.return)) _a.call(fonts_1);
            }
            finally { if (e_1) throw e_1.error; }
        }
        return fontTable;
    };
    return FontTable;
}());

exports.FontTable = FontTable;
