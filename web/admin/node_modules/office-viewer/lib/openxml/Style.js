...
var parseTcPr = require('../word/parse/parseTcPr.js');
var OpenXML = require('../OpenXML.js');
var Paragraph = require('./word/Paragraph.js');
var Run = require('./word/Run.js');
var parseTablePr = require('../word/parse/parseTablePr.js');
var parseTrPr = require('../word/parse/parseTrPr.js');
...
function parseDefaultStyle(word, element) {
    ...
    if (rPrDefault) {
        ...
        if (rPr) {
            defaultStyle.rPr = Run.Run.parseRunPr(word, rPr);
        }
    }
    ...
    if (pPrDefault) {
        ...
        if (pPr) {
            defaultStyle.pPr = Paragraph.Paragraph.parseParagraphPr(word, pPr);
        }
    }
    return defaultStyle;
}
function parseTblStylePr(word, element) {
    ...
    try {
        for (var _b = tslib.__values(element.children), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            switch (tag) {
                case 'w:rPr':
                    style.rPr = Run.Run.parseRunPr(word, child);
                    break;
                case 'w:pPr':
                    style.pPr = Paragraph.Paragraph.parseParagraphPr(word, child);
                    break;
                case 'w:tblPr':
                    style.tblPr = parseTablePr.parseTablePr(word, child);
                    break;
                case 'w:tcPr':
                    style.tcPr = parseTcPr.parseTcPr(word, child);
                    break;
                case 'w:trPr':
                    style.trPr = parseTrPr.parseTrPr(word, child);
                    break;
            }
        }
    }
...
function parseStyle(word, element) {
    ...
    try {
        for (var _b = tslib.__values(element.children), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            switch (tag) {
                case 'w:name':
                    style.name = OpenXML.getVal(child);
                    break;
                case 'w:basedOn':
                    style.basedOn = OpenXML.getVal(child);
                    break;
                case 'w:rPr':
                case 'w:pPr':
                case 'w:tblPr':
...
function parseStyles(word, doc) {
    ...
    try {
        for (var stylesElement_1 = tslib.__values(stylesElement), stylesElement_1_1 = stylesElement_1.next(); !stylesElement_1_1.done; stylesElement_1_1 = stylesElement_1.next()) {
