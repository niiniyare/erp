...
var getViewRange = require('./getViewRange.js');
var Consts = require('../render/Consts.js');
var getViewPointData = require('./getViewPointData.js');
var getFrozenLeftViewPointRange = require('../render/cell/frozen/getFrozenLeftViewPointRange.js');
var getFrozenTopLeftViewPointRange = require('../render/cell/frozen/getFrozenTopLeftViewPointRange.js');
var getFrozenTopViewPointRange = require('../render/cell/frozen/getFrozenTopViewPointRange.js');
var hitTest = require('../render/selection/hitTest.js');
var Range = require('../io/excel/util/Range.js');
var Rect = require('../render/Rect.js');
var applyConditionalFormat = require('./applyConditionalFormat.js');
var RangeCache = require('./RangeCache.js');
var applyTablePartsStyle = require('./applyTablePartsStyle.js');
var getCellAbsolutePosition = require('./getCellAbsolutePosition.js');
var getAbsoluteAnchorPosition = require('../render/drawing/getAbsoluteAnchorPosition.js');
var getOneCellAnchorPosition = require('../render/drawing/getOneCellAnchorPosition.js');
var getTwoCellAnchorPosition = require('../render/drawing/getTwoCellAnchorPosition.js');
var applyAutoFilter = require('../data/applyAutoFilter.js');
...
var Sheet = /** @class */ (function () {
    function Sheet(sheetIndex, dataProvider, sheetData, workbook, renderOptions) {
        var _a, _b;
        this.rangeCache = new RangeCache.RangeCache();
        /**
         * 水平滚动条位置
         */
        this.scrollLeft = 0;
        ...
        // 初始按两个字符宽度来计算
        this.rowHeaderWidth = 2 * this.defaultFontSize.width + 4 * Consts.PADDING_SIZE;
    ...
    Sheet.prototype.hitTest = function (offsetX, offsetY) {
        ...
        var hitTestResult = hitTest.hitTest(offsetX, offsetY, rowHeaderWidth, colHeaderHeight, gridLineHitRange, this.dataViewRange, this.frozenViewRange, this.getMergeCells());
    ...
    Sheet.prototype.hitTestDrawing = function (offsetX, offsetY) {
        ...
        if (drawing) {
            try {
                for (var _d = tslib.__values(drawing.absoluteAnchors), _e = _d.next(); !_e.done; _e = _d.next()) {
                    var absoluteAnchor = _e.value;
                    var position = this.absolutePositionToRelativePosition(getAbsoluteAnchorPosition.getAbsoluteAnchorPosition(absoluteAnchor));
                    if (Rect.pointInRect(offsetX, offsetY, position)) {
                        return tslib.__assign(tslib.__assign({ type: 'drawing', drawing: absoluteAnchor, region: 'normal' }, fakeCell), position);
                    }
                }
            }
            ...
            try {
                for (var _f = tslib.__values(drawing.oneCellAnchors), _g = _f.next(); !_g.done; _g = _f.next()) {
                    var oneCellAnchor = _g.value;
                    var position = getOneCellAnchorPosition.getOneCellAnchorPosition(oneCellAnchor, this);
                    if (Rect.pointInRect(offsetX, offsetY, position)) {
                        return tslib.__assign(tslib.__assign({ type: 'drawing', drawing: oneCellAnchor, region: 'normal' }, fakeCell), position);
                    }
                }
            }
            ...
            try {
                for (var _h = tslib.__values(drawing.twoCellAnchors), _j = _h.next(); !_j.done; _j = _h.next()) {
                    var twoCellAnchor = _j.value;
                    var position = getTwoCellAnchorPosition.getTwoCellAnchorPosition(twoCellAnchor, this);
                    if (Rect.pointInRect(offsetX, offsetY, position)) {
                        return tslib.__assign(tslib.__assign({ type: 'drawing', drawing: twoCellAnchor, region: 'normal' }, fakeCell), position);
                    }
                }
            }
    ...
    Sheet.prototype.updateRowHeaderWidth = function (viewRange) {
        ...
        this.rowHeaderWidth =
            String(lastIndex).length * this.defaultFontSize.width + 4 * Consts.PADDING_SIZE;
    ...
    Sheet.prototype.getCellValueByRanges = function (ranges, includeHidden) {
        ...
        try {
            for (var ranges_1 = tslib.__values(ranges), ranges_1_1 = ranges_1.next(); !ranges_1_1.done; ranges_1_1 = ranges_1.next()) {
                ...
                result.push.apply(result, tslib.__spreadArray([], tslib.__read(this.getCellValueByRange(range, includeHidden)), false));
            }
        }
    ...
    Sheet.prototype.getViewPointData = function (viewRange) {
        var _this = this;
        return getViewPointData.getViewPointData(function (row) {
    ...
    Sheet.prototype.getViewPointRange = function (width, height) {
        var _this = this;
        return getViewRange.getViewRange(this.scrollLeft, this.scrollTop, this.rowHeaderWidth, this.colHeaderHeight, height, width, function (index) { return _this.dataProvider.getRowHeight(_this.sheetIndex, index); }, this.rowPositionCache, function (index) { return _this.dataProvider.getColWidth(_this.sheetIndex, index); }, this.colPositionCache, this.dataProvider.getColHiddenRange(this.sheetIndex));
    };
    Sheet.prototype.getFrozenTopLeftViewPointRange = function (xSplit, ySplit) {
        var _this = this;
        return getFrozenTopLeftViewPointRange.getFrozenTopLeftViewPointRange(xSplit, ySplit, this.rowHeaderWidth, this.colHeaderHeight, function (index) { return _this.dataProvider.getRowHeight(_this.sheetIndex, index); }, function (index) { return _this.dataProvider.getColWidth(_this.sheetIndex, index); });
    };
    Sheet.prototype.getFrozenTopViewPointRange = function (ySplit, width) {
        var _this = this;
        return getFrozenTopViewPointRange.getFrozenTopViewPointRange(ySplit, width, this.scrollLeft, this.rowHeaderWidth, this.colHeaderHeight, function (index) { return _this.dataProvider.getRowHeight(_this.sheetIndex, index); }, function (index) { return _this.dataProvider.getColWidth(_this.sheetIndex, index); }, this.colPositionCache, this.dataProvider.getColHiddenRange(this.sheetIndex));
    };
    Sheet.prototype.getFrozenLeftViewPointRange = function (xSplit, height) {
        var _this = this;
        return getFrozenLeftViewPointRange.getFrozenLeftViewPointRange(xSplit, height, this.scrollTop, this.rowHeaderWidth, this.colHeaderHeight, function (index) { return _this.dataProvider.getRowHeight(_this.sheetIndex, index); }, function (index) { return _this.dataProvider.getColWidth(_this.sheetIndex, index); }, this.rowPositionCache);
    };
    /**
     * 获取单元格样式及数据
     */
    Sheet.prototype.getCellInfo = function (row, col) {
        ...
        applyConditionalFormat.applyConditionalFormat(this, cellInfo, row, col);
        applyTablePartsStyle.applyTablePartsStyle(this, cellInfo);
        return cellInfo;
    };
    ...
    Sheet.prototype.getCellPosition = function (row, col) {
        var _this = this;
        var cellAbsolutePosition = getCellAbsolutePosition.getCellAbsolutePosition(row, col, function (index) { return _this.dataProvider.getRowHeight(_this.sheetIndex, index); }, this.rowPositionCache, function (index) { return _this.dataProvider.getColWidth(_this.sheetIndex, index); }, this.colPositionCache);
        return {
    ...
    Sheet.prototype.getMaxDrawingHeightAndWidth = function () {
        ...
        if (drawing) {
            ...
            try {
                for (var _d = tslib.__values(drawing.absoluteAnchors), _e = _d.next(); !_e.done; _e = _d.next()) {
                    var absoluteAnchor = _e.value;
                    var size = getAbsoluteAnchorPosition.getAbsoluteAnchorPosition(absoluteAnchor);
            ...
            try {
                for (var _f = tslib.__values(drawing.oneCellAnchors), _g = _f.next(); !_g.done; _g = _f.next()) {
                    var oneCellAnchor = _g.value;
                    var size = getOneCellAnchorPosition.getOneCellAnchorPosition(oneCellAnchor, this);
                    maxWidth = Math.max(maxWidth, size.x + size.width);
                    maxHeight = Math.max(maxHeight, size.y + size.height);
                }
            }
            ...
            try {
                for (var _h = tslib.__values(drawing.twoCellAnchors), _j = _h.next(); !_j.done; _j = _h.next()) {
                    var twoCellAnchor = _j.value;
                    var size = getTwoCellAnchorPosition.getTwoCellAnchorPosition(twoCellAnchor, this);
                    maxWidth = Math.max(maxWidth, size.x + size.width);
                    maxHeight = Math.max(maxHeight, size.y + size.height);
                }
            }
    ...
    Sheet.prototype.isMergeCell = function (mergeCell) {
        return Range.isMergeCell(mergeCell, this.getMergeCells());
    };
    ...
    Sheet.prototype.getSparklineGroups = function () {
        ...
        var _b;
        try {
            for (var _c = tslib.__values(((_b = this.getExtLst()) === null || _b === void 0 ? void 0 : _b.ext) || []), _d = _c.next(); !_d.done; _d = _c.next()) {
                var ext = _d.value;
    ...
    Sheet.prototype.applyAutoFilter = function (autoFilter, headerRowCount) {
        ...
        return applyAutoFilter.applyAutoFilter(this.sheetIndex, this.workbook.getWorkbookData(), this.sheetData, this.dataProvider, autoFilter, headerRowCount);
    };
