...
var presetIconSet = require('../../io/excel/preset/presetIconSet.js');
...
function iconSet(sheet, cellInfo, ranges, cfRule) {
    ...
    if (!rangeIconSet) {
        ...
        var icons = [];
        var presetIcon = presetIconSet.presetIconSet[iconSetName];
        if (!presetIcon) {
        ...
        try {
            for (var _c = tslib.__values(cfvo.entries()), _d = _c.next(); !_d.done; _d = _c.next()) {
                var _e = tslib.__read(_d.value, 2), i = _e[0], cfvoItem = _e[1];
                ...
                var _f = getMinMax.getMinMax(rangeValues), min = _f.min, max = _f.max;
    ...
    // 当前值所在的百分比
    var percent = number.getPercent(value, rangeIconSet.min, rangeIconSet.max);
    // 查找所属 icon
    var startIcon = rangeIconSet.icons[0].icon;
    try {
        for (var _g = tslib.__values(rangeIconSet.icons), _h = _g.next(); !_h.done; _h = _g.next()) {
            var icon = _h.value;
