...
function renderLine(ctx, width, height, min, max, data, sparklineOptions, getColor) {
    ...
    try {
        for (var _d = tslib.__values(data.entries()), _e = _d.next(); !_e.done; _e = _d.next()) {
            var _f = tslib.__read(_e.value, 2), index = _f[0], value = _f[1];
            if (value === undefined) {
    ...
    if (sparklineOptions.markers) {
        try {
            for (var points_1 = tslib.__values(points), points_1_1 = points_1.next(); !points_1_1.done; points_1_1 = points_1.next()) {
                var point = points_1_1.value;
                ctx.beginPath();
                applyColor.applyColor(ctx, data, point.index, point.value, min, max, sparklineOptions, color, getColor);
        ...
        try {
            // 画特定的点
            for (var points_2 = tslib.__values(points), points_2_1 = points_2.next(); !points_2_1.done; points_2_1 = points_2.next()) {
                var point = points_2_1.value;
                var showRender = applyColor.applyColor(ctx, data, point.index, point.value, min, max, sparklineOptions, color, getColor);
                if (!showRender) {
                    continue;
                }
                ctx.beginPath();
