...
var renderSparkline = require('./renderSparkline.js');
var getRangePosition = require('../selection/getRangePosition.js');
...
function drawSparkline(currentSheet, viewRange, canvas) {
    ...
    var viewRangeRef = Range.viewRangeToRangeRef(viewRange);
    ...
    var _loop_1 = function (sparklineGroup) {
        ...
        var _loop_2 = function (sparkline) {
            ...
            var sqrefCellRange = Range.parseRange(sqref);
            if (Range.rangeIntersect(viewRangeRef, sqrefCellRange)) {
                if (f.includes('!')) {
                    var cellDisplayData_1 = getRangePosition.getRangePosition(workbook, viewRange.region, sqrefCellRange);
                    if (!cellDisplayData_1) {
                        console.warn('找不到对应的单元格', sqref);
                        return "continue";
                    }
                    var _f = tslib.__read(f.split('!'), 2), sheetName = _f[0], rangeStr = _f[1];
                    // 获取范围内的数据
                    var dataRange = Range.parseRange(rangeStr);
                    ...
                    if (targetSheet) {
                        ...
                        canvas.customDrawWithCache(cacheKey, cellDisplayData_1.x + padding_1, cellDisplayData_1.y + padding_1, cellDisplayData_1.width - 2 * padding_1, cellDisplayData_1.height - 2 * padding_1, function (ctx) {
                            renderSparkline.renderSparkline(ctx, cellDisplayData_1.width - 2 * padding_1, cellDisplayData_1.height - 2 * padding_1, data_1, sparklineGroup, function (color) {
        ...
        try {
            for (var _d = (e_2 = void 0, tslib.__values(((_b = sparklineGroup['x14:sparklines']) === null || _b === void 0 ? void 0 : _b['x14:sparkline']) || [])), _e = _d.next(); !_e.done; _e = _d.next()) {
    ...
    try {
        for (var sparklineGroups_1 = tslib.__values(sparklineGroups), sparklineGroups_1_1 = sparklineGroups_1.next(); !sparklineGroups_1_1.done; sparklineGroups_1_1 = sparklineGroups_1.next()) {
