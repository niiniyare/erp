...
function autoWrapText(ctx, text, width, fontStyle) {
    ...
    if (typeof text === 'string') {
        ...
        var _loop_1 = function (t) {
            var newTokens = tokenizer.tokenizer(t.t);
        ...
        try {
            for (var text_1 = tslib.__values(text), text_1_1 = text_1.next(); !text_1_1.done; text_1_1 = text_1.next()) {
    ...
    var currentWidth = 0;
    // 计算默认字体高度
    var defaultSize = measureTextWithCache.measureTextWithCache(ctx, defaultFont, '1');
    ...
    try {
        for (var tokens_1 = tslib.__values(tokens), tokens_1_1 = tokens_1.next(); !tokens_1_1.done; tokens_1_1 = tokens_1.next()) {
            ...
            if (token.rPr && Object.keys(token.rPr).length > 0) {
                font = genFontStr.genFontStrFromRPr(token.rPr, fontStyle);
            }
            var size = measureTextWithCache.measureTextWithCache(ctx, font, token.t);
            ...
            if (tokenWidth > width) {
                ...
                try {
                    for (var _d = (e_3 = void 0, tslib.__values(stringToArray.stringToArray(token.t))), _e = _d.next(); !_e.done; _e = _d.next()) {
                        var char = _e.value;
                        // 避免太多 token，这里会对 token 进行合并
                        var charSize = measureTextWithCache.measureTextWithCache(ctx, font, char);
                        var charWidth = charSize.width;
                        if (currentWidth + charWidth > width) {
                            var splitToken = tslib.__assign(tslib.__assign({}, token), { w: currentSplitWidth, t: currentText });
...
exports.autoWrapText = autoWrapText;
