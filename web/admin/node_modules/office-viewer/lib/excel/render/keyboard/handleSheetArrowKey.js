...
function handleSheetCanvasKeydown(e, workbook) {
    ...
    if (e.key === 'ArrowDown') {
        activeCell.startRow = Math.min(Consts.MAX_ROW, activeCell.startRow + 1);
        changeActiveCell = true;
    }
    if (e.key === 'ArrowLeft') {
    ...
    if (e.key === 'ArrowRight') {
        activeCell.startCol = Math.min(Consts.MAX_COL, activeCell.startCol + 1);
        changeActiveCell = true;
    }
    if (changeActiveCell) {
        e.preventDefault();
...
exports.handleSheetCanvasKeydown = handleSheetCanvasKeydown;
