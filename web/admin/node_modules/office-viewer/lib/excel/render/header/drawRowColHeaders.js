'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var tslib = require('tslib');
var numberToLetters = require('../../io/excel/util/numberToLetters.js');
var genFontStr = require('../cell/genFontStr.js');

/**
 * 绘制行列的表头
 */
function drawRowColHeaders(currentSheet, viewRange, sheetCanvas, renderOptions, defaultFontSize, defaultFontStyle) {
    var e_1, _a, e_2, _b;
    if (currentSheet.showRowColHeaders() === false) {
        return;
    }
    var rows = viewRange.rows, startRowOffset = viewRange.startRowOffset, height = viewRange.height, width = viewRange.width, cols = viewRange.cols, startColOffset = viewRange.startColOffset;
    var _c = currentSheet.getRowColSize(), rowHeaderWidth = _c.rowHeaderWidth, colHeaderHeight = _c.colHeaderHeight;
    var gridLineColor = renderOptions.gridLineColor, rowColHeadersBgColor = renderOptions.rowColHeadersBackgroundColor, hiddenRowColHeadersLineColor = renderOptions.hiddenRowColHeadersLineColor;
    var currentRowOffset = startRowOffset;
    // 竖向的背景
    sheetCanvas.drawRect(0, 0, rowHeaderWidth, height, rowColHeadersBgColor);
    var font = genFontStr.genFontStr(defaultFontStyle);
    var color = renderOptions.rowColHeadersColor;
    var hasHiddenRow = false;
    try {
        // 绘制竖向的线表头
        for (var rows_1 = tslib.__values(rows), rows_1_1 = rows_1.next(); !rows_1_1.done; rows_1_1 = rows_1.next()) {
            var i = rows_1_1.value;
            // 绘制竖向的文字
            var rowHeight = currentSheet.getRowHeight(i);
            // 为零就代表隐藏，标记一下
            if (rowHeight === 0) {
                hasHiddenRow = true;
                continue;
            }
            // 横向的线
            var lineY = currentRowOffset + colHeaderHeight;
            sheetCanvas.drawLine({
                x1: 0,
                y1: lineY,
                x2: rowHeaderWidth,
                y2: lineY
            }, gridLineColor);
            // 如果有隐藏内容的情况
            if (hasHiddenRow) {
                // 重置一下
                hasHiddenRow = false;
                sheetCanvas.drawLine({
                    x1: 0,
                    y1: lineY,
                    x2: rowHeaderWidth,
                    y2: lineY
                }, hiddenRowColHeadersLineColor, 3);
            }
            else {
                sheetCanvas.drawLine({
                    x1: 0,
                    y1: lineY,
                    x2: rowHeaderWidth,
                    y2: lineY
                }, gridLineColor);
            }
            var textSize = String(i + 1).length;
            var textWidth = textSize * defaultFontSize.width;
            sheetCanvas.drawText(font, color, String(i + 1), (rowHeaderWidth - textWidth) / 2, currentRowOffset +
                (rowHeight - defaultFontSize.height) / 2 +
                colHeaderHeight, 'top');
            currentRowOffset += rowHeight;
        }
    }
    catch (e_1_1) { e_1 = { error: e_1_1 }; }
    finally {
        try {
            if (rows_1_1 && !rows_1_1.done && (_a = rows_1.return)) _a.call(rows_1);
        }
        finally { if (e_1) throw e_1.error; }
    }
    // 绘制最左侧的线，目前 office 365 是不绘制的
    // sheetCanvas.drawLine(
    //   {
    //     x1: 0,
    //     y1: 0,
    //     x2: 0,
    //     y2: currentRowOffset + colHeaderHeight
    //   },
    //   gridLineColor
    // );
    // 横向的背景
    sheetCanvas.drawRect(0, 0, width, colHeaderHeight, rowColHeadersBgColor);
    var currentColOffset = startColOffset;
    // 横向字体的 Y 坐标，这个是固定的
    var colTextY = (colHeaderHeight - defaultFontSize.height) / 2;
    // 绘制横向的线表头
    // 上一个列的索引
    var lastColIndex = 0;
    try {
        for (var cols_1 = tslib.__values(cols), cols_1_1 = cols_1.next(); !cols_1_1.done; cols_1_1 = cols_1.next()) {
            var i = cols_1_1.value;
            var colWidth = currentSheet.getColWidth(i);
            // 竖向的线
            var lineX = currentColOffset + rowHeaderWidth;
            // 如果当前索引比上一个索引大 1 以上，就代表有隐藏的列
            var hasHiddenCol = i - lastColIndex > 1;
            if (hasHiddenCol) {
                sheetCanvas.drawLine({
                    x1: lineX,
                    y1: 0,
                    x2: lineX,
                    y2: colHeaderHeight
                }, hiddenRowColHeadersLineColor, 3);
            }
            else {
                sheetCanvas.drawLine({
                    x1: lineX,
                    y1: 0,
                    x2: lineX,
                    y2: colHeaderHeight
                }, gridLineColor);
            }
            // 绘制横向的文字
            var text = numberToLetters.numberToLetters(i);
            var textSize = text.length;
            var textWidth = textSize * defaultFontSize.width;
            sheetCanvas.drawText(font, color, text, currentColOffset + (colWidth - textWidth) / 2 + rowHeaderWidth, colTextY, 'top');
            currentColOffset += colWidth;
            lastColIndex = i;
        }
    }
    catch (e_2_1) { e_2 = { error: e_2_1 }; }
    finally {
        try {
            if (cols_1_1 && !cols_1_1.done && (_b = cols_1.return)) _b.call(cols_1);
        }
        finally { if (e_2) throw e_2.error; }
    }
    // 绘制最上方的线，目前 office 365 是不绘制的
    // sheetCanvas.drawLine(
    //   {
    //     x1: 0,
    //     y1: 0,
    //     x2: currentColOffset + rowHeaderWidth,
    //     y2: 0
    //   },
    //   gridLineColor
    // );
    // 绘制左上角的背景，TODO: 这里还有别的设计，比如一个倒三角
    sheetCanvas.drawRect(0, 0, rowHeaderWidth, colHeaderHeight, rowColHeadersBgColor);
}

exports.drawRowColHeaders = drawRowColHeaders;
