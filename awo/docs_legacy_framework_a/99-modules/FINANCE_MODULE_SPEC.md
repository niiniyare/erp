...
| `finance.ledger_entry.read` | Read ledger entries (read-only entity) |
...
**Role-to-permission mapping** (managed by IAM module, not by EntityDefinition):

| Role | Permissions Granted |
|------|---------------------|
| `role:finance.viewer` | All `*.read` permissions |
| `role:finance.accounts_payable` | `finance.invoice.*`, `finance.payment.*`, `finance.customer.*`, plus all read |
| `role:finance.manager` | All finance permissions including `approve`, `post`, `journal_entry.*` |
| `role:tenant.admin` | All finance permissions plus delete |
