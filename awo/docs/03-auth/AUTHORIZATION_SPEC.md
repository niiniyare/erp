...
`PermissionSet` on `EntityDefinition`. Module authors declare **permission identifiers** — stable names for capabilities (e.g., `"finance.invoice.create"`). No roles, no subjects, no backend constructs. Pure Go struct. Fully engine-agnostic.
