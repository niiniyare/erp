...
All CRITICAL application-layer bugs and the majority of schema, query, and infrastructure issues have been fixed. Below is the full fix ledger.
...
| CRIT-08 | Reversal has no wrapping DB transaction | **PARTIALLY FIXED** — best-effort cleanup on failure; true DB txn requires repo-layer support |
...
1. **CRIT-08** — true reversal atomicity requires repo-layer DB transaction support (currently best-effort); architectural change
2. **Tests** — zero unit/integration tests for all critical paths (see review §7)
