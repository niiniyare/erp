...
var SystemConfig = ResultType("application/vnd.erp.config", func() {
	...
	Attributes(func() {
		...
		Field(24, "change_history", ArrayOf(Type("ConfigChangeRecord", func() {
		...
		})), "Configuration change history", func() {
...
var ConfigTemplate = Type("ConfigTemplate", func() {
	...
	Field(6, "configurations", ArrayOf(Type("TemplateConfig", func() {
	...
	})), "Template configuration definitions", func() {
		Description("Set of related configurations in this template")
	})
	
	Field(7, "variables", ArrayOf(Type("TemplateVariable", func() {
		...
		Field(5, "allowed_values", ArrayOf(Any), "Allowed variable values", func() {
			Example([]any{"development", "staging", "production"})
		})
		Field(6, "is_required", Boolean, "Required variable", func() {
			Default(true)
		})
		Required("name", "description", "data_type", "is_required")
	})), "Template variables for customization", func() {
...
var ConfigValidation = Type("ConfigValidation", func() {
	...
	Field(5, "messages", ArrayOf(Type("ValidationMessage", func() {
		Field(1, "level", String, "Message severity", func() {
			Enum("INFO", "WARNING", "ERROR", "CRITICAL")
			Example("ERROR")
		})
		Field(2, "code", String, "Error/warning code", func() {
			Pattern("^[A-Z][A-Z0-9_]{2,19}$")
			Example("SMTP_UNREACHABLE")
		})
		Field(3, "message", String, "Human-readable message", func() {
			MaxLength(300)
			Example("Unable to connect to SMTP server at smtp.mailserver.com:587")
		})
		Field(4, "field", String, "Related configuration field", func() {
			Example("server_host")
		})
		Field(5, "suggestion", String, "Suggested resolution", func() {
			MaxLength(200)
			Example("Verify server hostname and network connectivity")
		})
		Required("level", "code", "message")
	})), "Validation messages and errors", func() {
...
var ConfigDefinition = Type("ConfigDefinition", func() {
	...
	Field(20, "dependencies", ArrayOf(Type("ConfigDependency", func() {
		Field(1, "config_key", String, "Dependent configuration key", func() {
			Pattern("^[a-z][a-z0-9_.]{2,199}$")
			Example("email.smtp.port")
		})
		Field(2, "dependency_type", String, "Type of dependency", func() {
			Enum("REQUIRES", "CONFLICTS", "IMPLIES", "CONDITIONAL")
			Example("REQUIRES")
		})
		Field(3, "condition", String, "Dependency condition", func() {
			Example("when email.enabled = true")
		})
		Field(4, "description", String, "Dependency description", func() {
			MaxLength(200)
			Example("SMTP port must be configured when server host is set")
		})
		Required("config_key", "dependency_type")
	})), "Configuration dependencies", func() {
