...
func main() {
	...
	form.business(cfg)
	for _, b := range form.ALL {
		fmt.Printf("ID: %d, Description: %s, Discount: %.2f\n", b.ID, b.Description, b.Discount)
	}

	// Get a single business
	form.ID = 1
	form.getBusiness(cfg)
	fmt.Printf("Business ID %d: %s with discount %.2f\n", form.ID, form.Description, form.Discount)

	// Save a new business
	form.ID = 0
	form.Description = "New Business"
	form.Discount = 10 // 10%
	form.saveBusiness(cfg)

	// Update existing business
	form.ID = 1
	form.Description = "Updated Business"
	form.Discount = 15 // 15%
	form.saveBusiness(cfg)

	// Delete a business
	form.ID = 2
	form.deleteBusiness(cfg)
}
