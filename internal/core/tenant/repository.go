...
func (r *repository) ProvisionTenant(ctx context.Context, req ProvisionTenantRequest) (*ProvisionedTenantInfo, error) {
	...
	logger.InfoContext(ctx, "Attempting to provision tenant within a transaction", logger.Fields{
		"name":          req.Name,
		"email":         req.Email,
		"subdomain":     req.Subdomain,
		"currency_code": req.CurrencyCode,
		"operation":     "repository.ProvisionTenant.start",
	})

	var provisionedInfo *ProvisionedTenantInfo
	...
	err := r.store.WithTx(ctx, func(ctx context.Context, txStore db.Store) error {
		...
		provisionedID, err := txStore.ProvisionTenant(ctx, db.ProvisionTenantParams{
			PName:         req.Name,
			PEmail:        req.Email,
		...
		if err != nil {
			logger.ErrorContext(ctx, "Error from provision_tenant_complete DB function", logger.Fields{
				"name":      req.Name,
				"error":     err.Error(),
				"operation": "repository.ProvisionTenant.db_call",
			})
			return fmt.Errorf("failed to provision tenant via DB function: %w", err)
		}

		logger.InfoContext(ctx, "Successfully received ID from DB function", logger.Fields{
			"provisioned_id": provisionedID.String(),
		})

		provisionedInfo = &ProvisionedTenantInfo{
			TenantID:  provisionedID,
			Slug:      "", // Slug is generated by DB trigger, not returned by function
			Subdomain: req.Subdomain,
			Status:    "pending",
		}
		return nil
	...
	if err != nil {
		...
		logger.ErrorContext(ctx, "Tenant provisioning transaction failed", logger.Fields{
			"name":      req.Name,
			"error":     err.Error(),
			"operation": "repository.ProvisionTenant.transaction",
		})
		return nil, err
	}

	span.SetAttributes(attribute.String("tenant.provisioned_id", provisionedInfo.TenantID.String()))
	logger.InfoContext(ctx, "Tenant provisioned successfully", logger.Fields{
		"tenant_id": provisionedInfo.TenantID.String(),
		"operation": "repository.ProvisionTenant.success",
	})
