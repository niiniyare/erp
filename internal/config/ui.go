...
type UIServerConfig struct {
	...
	// TLS Configuration
	TLS UITLSConfig `yaml:"tls" json:"tls" mapstructure:"tls"`
}
...
type UIJWTConfig struct {
	...
	ValidateAudience bool          `yaml:"validate_audience" json:"validate_audience" mapstructure:"validate_audience"`
	ValidateIssuer   bool          `yaml:"validate_issuer" json:"validate_issuer" mapstructure:"validate_issuer"`
}
...
type UISessionConfig struct {
	...
	RedisURL  string `yaml:"redis_url" json:"redis_url" mapstructure:"redis_url"`
	RedisDB   int    `yaml:"redis_db" json:"redis_db" mapstructure:"redis_db"`
...
func SetUIDefaults(v *viper.Viper) {
	v.SetDefault("ui.server.port", "8080")
	v.SetDefault("ui.server.host", "0.0.0.0")
	v.SetDefault("ui.server.read_timeout", 30*time.Second)
	v.SetDefault("ui.server.write_timeout", 30*time.Second)
	...
	v.SetDefault("ui.server.tls.enabled", false)
	v.SetDefault("ui.server.tls.min_version", "1.2")
	...
	v.SetDefault("ui.security.jwt.algorithm", "RS256")
	v.SetDefault("ui.security.jwt.issuer", "awo-erp")
	...
	v.SetDefault("ui.security.jwt.clock_skew", 5*time.Minute)
	v.SetDefault("ui.security.jwt.validate_audience", true)
	v.SetDefault("ui.security.jwt.validate_issuer", true)
	...
	v.SetDefault("ui.security.encryption.key_derivation", "pbkdf2")
	v.SetDefault("ui.security.encryption.force_https", true)
	v.SetDefault("ui.security.encryption.hsts_max_age", 31536000)

	v.SetDefault("ui.session.store", "redis")
	v.SetDefault("ui.session.redis_db", 0)
	v.SetDefault("ui.session.key_prefix", "ui:session:")
...
func BindUIEnvVars(v *viper.Viper) {
	v.BindEnv("ui.server.port", "UI_PORT")
	v.BindEnv("ui.server.host", "UI_HOST")
	...
	v.BindEnv("ui.server.tls.enabled", "UI_TLS_ENABLED")
	v.BindEnv("ui.server.tls.cert_file", "UI_TLS_CERT_FILE")
	v.BindEnv("ui.server.tls.key_file", "UI_TLS_KEY_FILE")
	v.BindEnv("ui.security.jwt.public_key_path", "JWT_PUBLIC_KEY_PATH")
	v.BindEnv("ui.security.jwt.private_key_path", "JWT_PRIVATE_KEY_PATH")
	v.BindEnv("ui.security.jwt.issuer", "JWT_ISSUER")
	v.BindEnv("ui.security.jwt.audience", "JWT_AUDIENCE")
