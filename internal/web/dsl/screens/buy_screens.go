...
// GoodsReceiptScreen composes the goods receipt note form.
func GoodsReceiptScreen(sess ui.UISessionContext) ast.Node {
	lineCfg := blocks.GRNLineItemConfig()
	return ast.PageNode{
		Title: "New Goods Receipt",
		InitAPI: &ast.APISpec{
			Method: "get",
			URL:    "/api/v1/buy/goods-receipts/:id",
			SendOn: "${params.id}",
		},
		Body: []ast.Node{
			blocks.DocumentHeaderBlock(sess, blocks.DocumentHeaderConfig{
			...
			ast.SectionNode{
				...
				Body: []ast.Node{
					ast.SelectNode{
						...
						Label:    "Against PO",
						Source:   &ast.APISpec{Method: "get", URL: "/api/v1/buy/purchase-orders/options"},
						Searchable: true,
					},
					ast.InputDateNode{Name: "received_date", Label: "Received Date", Required: true},
					ast.InputTextNode{Name: "warehouse", Label: "Receiving Warehouse"},
				},
			},
			blocks.ProductServiceLineBlock(sess, lineCfg),
			blocks.AttachmentsBlock(sess),
