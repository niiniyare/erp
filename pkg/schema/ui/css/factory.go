...
// GetPropertyRegistry returns the property registry used by this factory
func (f *Factory) GetPropertyRegistry() *PropertyRegistry {
	return f.propertyRegistry
}

// ValidateProperty validates a CSS property value using the schema-based registry
func (f *Factory) ValidateProperty(propertyName, value string) error {
	if f.propertyRegistry == nil {
		return fmt.Errorf("property registry not initialized")
	}
	return f.propertyRegistry.ValidateProperty(propertyName, value)
}

// GetAvailableProperties returns all available CSS properties
func (f *Factory) GetAvailableProperties() []string {
	if f.propertyRegistry == nil {
		return nil
	}
	return f.propertyRegistry.GetAllPropertyNames()
}

// GetPropertyDefinition returns the definition for a CSS property
func (f *Factory) GetPropertyDefinition(propertyName string) (*PropertyDefinition, bool) {
	if f.propertyRegistry == nil {
		return nil, false
	}
