...
WHERE id = sqlc.arg('transaction_id') 
  AND tenant_id = current_tenant_id()
  AND deleted_at IS NULL;

-- name: GetTransactionByNumber :one
SELECT * FROM finance_transactions
WHERE transaction_number = sqlc.arg('transaction_number') 
  AND tenant_id = current_tenant_id()
  AND deleted_at IS NULL;
...
WHERE tenant_id = current_tenant_id()
  AND deleted_at IS NULL
  AND (sqlc.narg('transaction_type')::VARCHAR IS NULL OR transaction_type = sqlc.narg('transaction_type')::VARCHAR)
  AND (sqlc.narg('transaction_status')::VARCHAR IS NULL OR transaction_status = sqlc.narg('transaction_status')::VARCHAR)
...
SET 
    transaction_status = COALESCE(sqlc.narg('transaction_status'), transaction_status),
    ...
    total_debit_amount = COALESCE(sqlc.narg('total_debit_amount'), total_debit_amount),
    total_credit_amount = COALESCE(sqlc.narg('total_credit_amount'), total_credit_amount),
    ...
    transaction_attributes = COALESCE(sqlc.narg('transaction_attributes'), transaction_attributes),
...
SELECT * FROM finance_transactions
WHERE source_document_type = sqlc.arg('source_document_type') 
...
WHERE tenant_id = current_tenant_id()
  AND deleted_at IS NULL
  AND (
    transaction_number ILIKE '%' || sqlc.arg('search_term') || '%' OR
    description ILIKE '%' || sqlc.arg('search_term') || '%' OR
    reference_number ILIKE '%' || sqlc.arg('search_term') || '%' OR
    external_reference ILIKE '%' || sqlc.arg('search_term') || '%' OR
    memo ILIKE '%' || sqlc.arg('search_term') || '%'
  )
ORDER BY 
  CASE WHEN transaction_number ILIKE sqlc.arg('search_term') || '%' THEN 1 ELSE 2 END,
...
UPDATE finance_transactions
SET 
    attachment_ids = array_append(COALESCE(attachment_ids, '{}'), sqlc.arg('attachment_id')),
    updated_at = NOW(),
    updated_by = sqlc.arg('updated_by')
WHERE id = sqlc.arg('transaction_id') 
  AND tenant_id = current_tenant_id()
