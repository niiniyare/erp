...
SET
  ...
  max_transactions_per_month = COALESCE(
    sqlc.narg('max_transactions_per_month'),
  ...
  fiscal_year_start_month = COALESCE(
    sqlc.narg('fiscal_year_start_month'),
...
SET
  ...
  updated_at = NOW()
WHERE
  id = ANY(sqlc.slice('tenant_ids')::UUID []);
...
WHERE
  (
    sqlc.narg('name_filter')::varchar IS NULL
  ...
  AND (
    ...
    OR industry = sqlc.narg('industry_filter')
  )
  AND deleted_at IS NULL
ORDER BY
  CASE
    WHEN sqlc.arg('sort_by')::varchar = 'name' THEN name
  END ASC,
  CASE
    WHEN sqlc.arg('sort_by')::varchar = 'created_at' THEN created_at
  END DESC,
  CASE
    WHEN sqlc.arg('sort_by')::varchar = 'updated_at' THEN updated_at
  END DESC,
  id DESC
LIMIT
...
SET
  password_policy = jsonb_build_object(
    'min_length',
    (sqlc.arg('min_length')::int),
    'require_uppercase',
    (sqlc.arg('require_uppercase')::bool),
    'require_lowercase',
    (sqlc.arg('require_lowercase')::bool),
    'require_numbers',
    (sqlc.arg('require_numbers')::bool),
    'require_symbols',
    (sqlc.arg('require_symbols')::bool)
  ),
  updated_at = NOW()
WHERE
  tenant_id = current_tenant_id();
...
UPDATE
  tenant_configurations
SET
  settings = sqlc.arg('settings')::jsonb,
  updated_at = NOW()
WHERE
  tenant_id = current_tenant_id();
...
SELECT
  settings ->> sqlc.arg('key')::text AS value
FROM
  tenant_configurations
WHERE
  tenant_id = current_tenant_id();
...
SELECT
  (settings ->> sqlc.arg('key')::text)::boolean AS value
FROM
  tenant_configurations
WHERE
  tenant_id = current_tenant_id();
...
SELECT
  (settings ->> sqlc.arg('key')::text)::integer AS value
FROM
  tenant_configurations
WHERE
  tenant_id = current_tenant_id();
