...
WHERE
  te.transaction_id = sqlc.arg('transaction_id')
...
WHERE
  te.account_id = sqlc.arg('account_id')
  AND te.tenant_id = current_tenant_id()
  AND te.deleted_at IS NULL
  AND (
    sqlc.narg('date_from')::date IS NULL
    OR t.transaction_date >= sqlc.narg('date_from')::date
  )
  AND (
    sqlc.narg('date_to')::date IS NULL
    OR t.transaction_date <= sqlc.narg('date_to')::date
  )
  AND (
    sqlc.narg('transaction_status')::transaction_status_enum IS NULL
    OR t.transaction_status = sqlc.narg('transaction_status')::transaction_status_enum
  )
ORDER BY
...
-- name: GetAccountTransactionBalance :one
SELECT
  account_id,
  SUM(
    CASE
...
WHERE
  ...
  AND (
    sqlc.narg('as_of_date')::date IS NULL
    OR t.posting_date <= sqlc.narg('as_of_date')
  )
GROUP BY
  account_id;
...
FROM
  ...
  JOIN finance_transactions t ON te.transaction_id = t.id
WHERE
  te.cost_center = sqlc.narg('cost_center')
  AND te.tenant_id = current_tenant_id()
  AND te.deleted_at IS NULL
  AND t.transaction_status = 'POSTED'
  AND (
    sqlc.narg('date_from')::date IS NULL
    OR t.transaction_date >= sqlc.narg('date_from')
  )
  AND (
    sqlc.narg('date_to')::date IS NULL
    OR t.transaction_date <= sqlc.narg('date_to')
  )
ORDER BY
  t.transaction_date DESC,
  te.entry_number ASC;
...
FROM
  ...
  JOIN finance_transactions t ON te.transaction_id = t.id
WHERE
  te.project_id = sqlc.narg('project_id')
  AND te.tenant_id = current_tenant_id()
  AND te.deleted_at IS NULL
  AND t.transaction_status = 'POSTED'
  AND (
...
FROM
  ...
  JOIN finance_transactions t ON te.transaction_id = t.id
WHERE
  te.department = sqlc.narg('department')
  AND te.tenant_id = current_tenant_id()
  AND te.deleted_at IS NULL
  AND t.transaction_status = 'POSTED'
  AND (
...
FROM
  ...
  AND (
    sqlc.narg('as_of_date')::date IS NULL
    OR t.posting_date <= sqlc.narg('as_of_date')::date
  )
WHERE
...
HAVING
  ...
  OR sqlc.arg('include_zero_balances') = TRUE
ORDER BY
  a.account_code ASC;
