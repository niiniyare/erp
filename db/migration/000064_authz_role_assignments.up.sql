CREATE TABLE role_assignments (
  ...
  granted_by    UUID,                                                    -- UUID of granting user; FK to users(id) added in 000412 (after users table exists)
...
CREATE INDEX idx_role_assignments_revoked ON role_assignments(revoked_at) WHERE revoked_at IS NOT NULL;
CREATE INDEX idx_role_assignments_granter ON role_assignments(granted_by) WHERE granted_by IS NOT NULL;
