...
COMMENT ON TABLE finance_account_groups IS 
...
CREATE INDEX idx_account_groups_tenant_code 
    ON finance_account_groups(tenant_id, group_code) 
    WHERE deleted_at IS NULL;

CREATE INDEX idx_account_groups_tenant_type 
    ON finance_account_groups(tenant_id, root_type, group_category) 
    WHERE deleted_at IS NULL;

-- Group hierarchy indexes
CREATE INDEX idx_account_groups_parent 
    ON finance_account_groups(parent_group_id) 
    WHERE deleted_at IS NULL;

CREATE INDEX idx_account_groups_hierarchy_level 
    ON finance_account_groups(tenant_id, group_level, parent_group_id) 
    WHERE deleted_at IS NULL;

-- Financial statement indexes
CREATE INDEX idx_account_groups_statement 
    ON finance_account_groups(tenant_id, financial_statement_section, statement_order) 
