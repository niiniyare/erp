...
GRANT SELECT, INSERT, UPDATE, DELETE ON finance_account_balances TO application_role, admin_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON finance_account_validation_rules TO application_role, admin_role;

-- Grant permissions on views (already created in previous migration)
GRANT SELECT ON v_finance_accounts_hierarchy TO application_role, admin_role;
GRANT SELECT ON v_finance_transaction_summary TO application_role, admin_role;
GRANT SELECT ON v_finance_account_activity TO application_role, admin_role;
