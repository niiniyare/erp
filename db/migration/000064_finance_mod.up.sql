...
SELECT
  ...
  EXISTS (
    SELECT
      1
    FROM
      finance_accounts children
    WHERE
      children.parent_account_id = a.id
...
COMMENT ON VIEW v_finance_accounts_with_groups IS 'Complete account view with group and header information for reporting';
...
FROM
  finance_account_groups g
  LEFT JOIN finance_accounts a ON (
    a.account_group_id = g.id
    OR a.account_header_id = g.id
  )
...
COMMENT ON VIEW v_financial_statement_structure IS 'Financial statement structure with account groups, hierarchies, and balances';
...
CREATE
OR REPLACE FUNCTION create_standard_account_groups(p_tenant_id UUID) RETURNS VOID AS
$$
DECLARE
v_assets_id UUID;
...
SELECT
  id INTO v_liabilities_id
FROM
  finance_account_groups
WHERE
  tenant_id = p_tenant_id
  AND group_code = 'LIABILITIES';
...
SELECT
  id INTO v_expenses_id
FROM
  finance_account_groups
WHERE
  tenant_id = p_tenant_id
  AND group_code = 'EXPENSES';
...
COMMENT ON FUNCTION create_standard_account_groups IS 'Creates standard account group structure for new tenants';
...
CREATE
OR REPLACE FUNCTION maintain_group_hierarchy_path() RETURNS TRIGGER AS
$$
DECLARE
...
IF NEW.parent_group_id IS NULL THEN NEW.group_path := '/' || NEW.group_code || '/';
...
SELECT
  group_path,
  group_level INTO parent_path,
  NEW.group_level
FROM
  finance_account_groups
WHERE
  id = NEW.parent_group_id;
...
CREATE TRIGGER trigger_maintain_group_hierarchy BEFORE
INSERT
  OR
UPDATE
  OF parent_group_id,
  group_code ON finance_account_groups FOR EACH ROW EXECUTE FUNCTION maintain_group_hierarchy_path();
...
SELECT
  ...
  -- Reporting flags
  a.show_in_reports
  AND COALESCE(g.show_in_summary, TRUE) AS include_in_reports
FROM
  finance_accounts a
  LEFT JOIN finance_account_groups g ON a.account_group_id = g.id
  LEFT JOIN finance_account_groups h ON a.account_header_id = h.id
...
COMMENT ON VIEW v_chart_of_accounts_complete IS 'Complete chart of accounts with full group and header hierarchy for reporting';

-- =====================================================================
-- FINANCIAL STATEMENT BUILDER VIEW
-- =====================================================================
CREATE VIEW v_financial_statement_builder AS WITH grouped_balances AS (
  SELECT
    coa.tenant_id,
  ...
  ROUND(
    (
      group_balance / NULLIF(
        SUM(group_balance) OVER (PARTITION BY tenant_id, statement_section),
        0
      )
    ) * 100,
    2
...
COMMENT ON VIEW v_financial_statement_builder IS 'Pre-aggregated data for building financial statements with grouping and totals';
...
CREATE
OR REPLACE FUNCTION update_group_balances(p_tenant_id UUID DEFAULT NULL) RETURNS VOID AS
$$
BEGIN
...
WITH group_totals AS (
  ...
  WHERE
    ...
    AND (
      a.account_group_id IS NOT NULL
      OR a.account_header_id IS NOT NULL
    )
  GROUP BY
    COALESCE(a.account_group_id, a.account_header_id)
)
UPDATE
  finance_account_groups g
SET
  group_attributes = COALESCE(g.group_attributes, '{}'::jsonb) || jsonb_build_object(
    'calculated_balance',
    gt.total_balance,
    'account_count',
    gt.account_count,
...
COMMENT ON FUNCTION update_group_balances IS 'Updates group metadata with calculated balances and account counts';

GRANT
SELECT
  ON v_finance_accounts_with_groups TO application_role,
  admin_role;

GRANT
SELECT
  ON v_chart_of_accounts_complete TO application_role,
  admin_role;

GRANT
SELECT
  ON v_financial_statement_builder TO application_role,
  admin_role;
