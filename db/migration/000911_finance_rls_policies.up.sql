...
ALTER TABLE finance_account_balances FORCE  ROW LEVEL SECURITY;

ALTER TABLE finance_account_validation_rules ENABLE ROW LEVEL SECURITY;
ALTER TABLE finance_account_validation_rules FORCE  ROW LEVEL SECURITY;

-- RLS policies for new tables
CREATE POLICY tenant_isolation_policy ON finance_account_balances FOR ALL TO application_role
  USING  (current_tenant_id() IS NOT NULL AND tenant_id = current_tenant_id())
  WITH CHECK (current_tenant_id() IS NOT NULL AND tenant_id = current_tenant_id());

CREATE POLICY tenant_isolation_policy ON finance_account_validation_rules FOR ALL TO application_role
  USING  (current_tenant_id() IS NOT NULL AND tenant_id = current_tenant_id())
  WITH CHECK (current_tenant_id() IS NOT NULL AND tenant_id = current_tenant_id());

-- Admin policies for new tables
CREATE POLICY admin_full_access_policy ON finance_account_balances
  FOR ALL TO admin_role USING (TRUE) WITH CHECK (TRUE);

CREATE POLICY admin_full_access_policy ON finance_account_validation_rules
  FOR ALL TO admin_role USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY finance_account_validation_rules_ro_select ON finance_account_validation_rules
  FOR SELECT TO readonly_role
  USING (current_tenant_id() IS NOT NULL AND tenant_id = current_tenant_id());
