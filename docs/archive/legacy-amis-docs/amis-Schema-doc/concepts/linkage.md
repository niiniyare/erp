...
awo-ui 会将返回的 `data` 写入表单数据域，因此下面的 `static` 组件就能显示了。
...
更改配置后，提交表单时，awo-ui 会寻找`target`所配置的目标组件，把`form`中所提交的数据，发送给该目标组件中，并将该数据**合并**到目标组件的数据域中，并触发目标组件的刷新操作，对于 CRUD 组件来说，刷新即重新拉取数据接口。
