...
如果无法知道数据总数，只能知道是否有下一页，请返回如下格式，awo-ui 会简单生成一个简单版本的分页控件。
...
awo-ui 只负责生成排序组件，并将排序参数传递给接口，而不会在前端对数据进行排序处理。参数格式如下：
...
awo-ui 只负责生成搜索组件，并将搜索参数传递给接口，而不会在前端对数据进行搜索处理。参数格式如下：
...
awo-ui 只负责生成下拉选择器组件，并将搜索参数传递给接口，而不会在前端对数据进行搜索处理。参数格式如下：
...
在 `headerToolbar` 和 `footerToolbar` 中可以配置各种 awo-ui 其它组件，比如按钮和 tpl：
...
这样 awo-ui 会在配置分页组件的地方，渲染出一个简单的页面跳转控件。
...
1. 批量操作按钮上配置 `disabledOn` 值为 `this.selectedItems.some(item => item.owner === this.awo-uiUser.name)`
2. 给表格加上 `itemCheckableOn` 值为 `this.owner === this.awo-uiUser.name` 表示只有 owner 是自己的才可以打勾。
...
- `insertAfter` 或者 `insertBefore` 这是 awo-ui 生成的 diff 信息，对象格式，key 为目标成员的 primaryField 值，即 id，value 为数组，数组中存放成员 primaryField 值。如：
