...
{
    "type": "iframe",
    "src": "https://github.com/baidu/awo-ui",
    "height": 300
}
```
...
```schema:
{
  "type": "page",
  "data": {
    "iframeSrc": "https://github.com/baidu/awo-ui"
  },
  "body": {
    "type": "iframe",
    "src": "${iframeSrc}",
...
- **type**: 当前事件类型
  - awo-ui:init：初始化的时候触发
  - awo-ui:update：组件更新时触发
  - awo-ui:receive：组件通过 target 接收到其他组件发送来数据的时候
- **data**：当前数据源数据

> 如果是 webpack 开发环境，注意过滤掉`webpackOk`类型消息

#### iframe 页面向 awo-ui 通信

可以通过以下两种方式实现 iframe 页面向 awo-ui 通信：

- 方式一：通过 events 属性，基于[Action](./action)实现，有一定的局限性。
- 方式二：通过 onEvent 属性，基于[事件动作](../../docs/concepts/event-action)实现，更灵活。

> 注意：如果同时配置了 events 和 onEvent，awo-ui 都会执行，且 onEvent 配置的动作行为会先于 events 执行。
...
- `type`: 标识要触发的 awo-ui 行为，请使用 `amis:xxx` 的格式，这里我们设置为配置好的`detail`事件
- `data`: 传给 awo-ui 的数据，amis 会将该数据与当前数据域进行合并进行使用

2. 在 awo-ui 的 iframe 组件中指明需要监听的消息名称，以及需要执行的动作。

```json
// 方式一：即在 awo-ui 的 iframe 配置项中配置 events 对象
{
  "type": "iframe",
  ...
  "events": {
    "detail": {
      ...
      "dialog": {
        "title": "弹框",
        "body": "iframe 传给 awo-ui 的 id 是：${iframeId}" // 在弹框中渲染`"iframe 传给 amis 的 id 是：${iframeId}"`这段模板，即111
      }
    }
  }
}

// 方式二：即在 awo-ui 的 iframe 配置项中配置 onEvent 对象
{
  "type": "iframe",
  "src": "http://www.xxx.com",
  "onEvent": {
    "detail": {
      "actions": [
        ...
        // 动作 2，触发指定组件的特性动作
        {
          ...
          "componentId": "form01",
          "data": {
            "iframeId": "${iframeId}" // 刷新请求参数为`"iframe 传给 awo-ui 的 id 是：${iframeId}"`这段模板，即111
          }
        }
      ]
    }
...
默认 awo-ui 中只支持给 iframe 配置固定高度，我们可以通过上面说到的通信机制实现高度自适应。

1. 首先在 iframe 页面中获取到页面高度
2. 通过`awo-ui:resize`事件，将高度信息发送给 amis
