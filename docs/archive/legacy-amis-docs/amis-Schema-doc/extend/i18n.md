...
```javascript
import 'awo-ui-ui/lib/locale/en-US';
```

在渲染 awo-ui 组件的时候设置 locale 为 en-US
...
awo-ui 渲染的 env 参数可以配置全局文本替换，能基于它实现多语言替换功能
...
let awo-uiScoped = amis.embed(
  ...
  {
    replaceText: {
      AMIS_HOST: 'https://baidu.gitee.io/awo-ui'
    }
  }
);
```
...
如果想扩展其他语言，首先参考 `https://github.com/baidu/awo-ui/blob/master/src/locale/en-US.ts` 文件，然后参考后面的示例注册新语言，未翻译的文字都将使用中文。

### JS SDK 扩展方法

```javascript
let awo-uiLib = amisRequire('amis');
awo-uiLib.registerLocale('jp', {
  'Form.submit': '送信'
});
```
...
```javascript
import {registerLocale} from 'awo-ui';
registerLocale('jp', {
  'Form.submit': '送信'
});
```
