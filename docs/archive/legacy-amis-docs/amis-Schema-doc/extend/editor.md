...
目前 awo-ui 可视化编辑器也作为单独的 npm 包发布了出来，可以通过 npm 安装使用。

在线体验：https://aisuda.github.io/awo-ui-editor-demo  
示例代码：https://github.com/aisuda/awo-ui-editor-demo

## 使用

目前有两个 npm 包：`awo-ui-editor` 和 `amis-editor-core`。

- `awo-ui-editor-core` 包含了少量底层必要的功能实现，里面没有包含 amis 内置渲染器插件的任何实现。
- `awo-ui-editor` 基于 `amis-editor-core` 实现了 amis 内置的所有渲染器的可视化编辑器插件。

如果你没有使用 awo-ui 内置渲染器，推荐只使用 `amis-editor-core`，否则推荐使用 `amis-editor`。这里主要介绍 `amis-editor`, `amis-editor-core` 的使用方式是一样的。
...
```jsx
import {Editor} from 'awo-ui-editor';
...
- `value: Schema` awo-ui json 配置，比如：`{type: 'page', body: 'contents...'}`
...
- `$schemaUrl?: string` 提供 awo-ui 产出的 schema.json 的访问路径。主要用来给代码编辑模式提供属性提示信息。
- `className?: string` 额外加个 css 类名，辅助样式定义。
- `schemas?: JSONSchemaObject` 用来定义有哪些全局变量，辅助编辑器格式化绑定全局数据。
- `theme?: string` awo-ui 主题
...
- `awo-uiEnv?: any` 这是是给 amis 的 Env 对象，具体请前往 [env 说明](../start/getting-started#env)
...
移动端预览，需要额外提供 iframe 页面，并且与编辑器建立连接。mountInIframe 前请确保自定义的 awo-ui 渲染器已经加载了，否则会出现自定义渲染器无法编辑的问题。
...
<body>
  ...
  <script type="module">
    ...
    import {mountInIframe} from 'awo-ui-editor';
...
开始之前，需要先自定义一个 awo-ui 渲染器，然后再添加编辑器插件，让这个自定义渲染器可以在编辑器中可编辑。

```jsx
import React from 'react';
import {Renderer} from 'awo-ui';
...
通过以上代码，awo-ui 配置中通过 `type` 指定为 `my-renderer` 即可启用此组件。
...
```jsx
import {BasePlugin} from 'awo-ui-editor';
...
import {registerEditorPlugin} from 'awo-ui-editor';
...
编辑器在渲染 awo-ui 配置的时候，会把所有的 json（配置） 节点都自动加个 `$$id` 唯一 id。然后复写了 `rendererResolver` 方法。某个节点 {type: 'xxxx'} 在找到对应 amis 组件渲染前，都会调用这个方法。
...
- `renderer` 渲染器信息，即注册 awo-ui 渲染器的时候注册的渲染器信息
...
通常右侧面板都是表单配置，使用 awo-ui 配置就可以完成。所以推荐的做法是，直接在这个插件上面定义 `panelBody` 或者 `panelBodyCreator` 即可。
...
这里先看简单的情况，比如 `container` 组件。它在 awo-ui 大概是这样实现的容器功能。通过 `this.props.render('body', schema)` 来实现的容器功能。
...
- `title` 脚手架框的标题
- `body` 表单项配置，参考 awo-ui 的 form 配置
- `mode` 表单默认展示方式，参考 awo-ui 的 form 配置
- `size` 弹窗大小，参考 awo-ui 的 dialog 配置
- `initApi` 初始化接口
- `api` 提交接口
- `validate` 整体验证钩子
