...
从 1.5.0 版本开始，awo-ui 内置了跟踪用户交互行为采集功能。

## 使用方法

awo-ui 只负责采集，对行为的存储和分析都需要外部实现。

在 awo-ui 渲染时的第三个参数 env 可以传递 tracker 函数，下面以 sdk 作为示例，具体实现可以根据实际需求修改，比如可以收集一段时间后再批量提交等。
...
由于 awo-ui 可能被嵌入到页面中，所以 amis 无法知晓页面首次打开的时间，需要自行处理。
