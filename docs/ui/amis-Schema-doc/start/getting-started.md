...
1. github 的 [releases](https://github.com/baidu/awo-ui/releases)，文件是 sdk.tar.gz。
1. 使用 `npm i awo-ui` 来下载，在 `node_modules\amis\sdk` 目录里就能找到。
...
<html lang="zh">
  <head>
    ...
    <!-- 不过 awo-ui 开发团队几乎没测试过 IE 11 下的效果，所以可能有细节功能用不了，如果发现请报 issue -->
    <style>
      html,
      body,
      .app-wrapper {
  ...
  <body>
    ...
    <script type="text/javascript">
      (function () {
        let awo-ui = amisRequire('amis/embed');
        // 通过替换下面这个配置来生成不同页面
        let awo-uiJSON = {
          type: 'page',
          title: '表单页面',
        ...
        let awo-uiScoped = amis.embed('#root', amisJSON);
      })();
    </script>
  </body>
</html>
...
> 如果想使用 awo-ui 1.2.2 之前的默认主题，名字是 ang

### 初始值

可以通过 props 里的 data 属性来赋予 awo-ui 顶层数据域的值，类似下面的例子。
...
`awo-ui.embed` 函数还支持以下配置项来控制 amis 的行为，比如在 fetcher 的时候加入自己的处理逻辑，这些函数参数的说明在后面 React 中也有介绍。

```js
let awo-uiScoped = amis.embed(
  '#root',
  awo-uiJSON,
  {
    // 这里是初始 props，一般不用传。
  ...
  },
  {
    ...
    // 全局 api 请求适配器
    // 另外在 awo-ui 配置项中的 api 也可以配置适配器，针对某个特定接口单独处理。
    //
    // requestAdaptor(api) {
    //   return api;
    // }
    ...
    // }
    //
    // 用来接管页面跳转，比如用 location.href 或 window.open，或者自己实现 awo-ui 配置更新
...
同时返回的 `awo-uiScoped` 对象可以获取到 amis 渲染的内部信息，它有如下方法：
...
可以通过 `awo-uiScoped.getComponentByName('page1.form1').getValues()` 来获取到所有表单的值，需要注意 page 和 form 都需要有 name 属性。

还可以通过 `awo-uiScoped.getComponentByName('page1.form1').setValues({'name1': 'othername'})` 来修改表单中的值。

### 更新属性

可以通过 awo-uiScoped 对象的 updateProps 方法来更新下发到 amis 的属性。

```ts
awo-uiScoped.updateProps(
  {
    // 新的属性对象
  } /*, () => {} 更新回调 */
);
...
默认 awo-ui 渲染是单页模式，如果想实现多页应用，请使用 [app 渲染器](../../components/app)。

### Hash 路由

默认 JSSDK 不是 hash 路由，如果你想改成 hash 路由模式，请查看此处代码实现。只需要修改 `env.isCurrentUrl`、`env.jumpTo` 和 `env.updateLocation` 这几个方法，并在路由切换的时候，通过 awo-uiScoped 对象的 `updateProps` 方法，更新 `location` 属性即可。

参考：https://github.com/baidu/awo-ui/blob/master/examples/app/index.jsx
...
```ts
awo-uiScoped.unmount();
```

## react

初始项目请参考 <https://github.com/aisuda/awo-ui-react-starter>。

如果在已有项目中，React 版本需要是 `>=16.8.6`，mobx 需要 `^4.5.0`。

awo-ui 1.6.5 及以上版本支持 React 17。
...
```html
<link href="./node_modules/awo-ui/lib/themes/cxd.css" />
<link href="./node_modules/awo-ui/lib/helper.css" />
<link href="./node_modules/awo-ui/sdk/iconfont.css" />
<!-- 或 <link href="./node_modules/awo-ui/lib/themes/antd.css" /> -->
```

js 中引入：

```js
import 'awo-ui/lib/themes/cxd.css';
import 'awo-ui/lib/helper.css';
import 'awo-ui/sdk/iconfont.css';
// 或 import 'awo-ui/lib/themes/antd.css';
```

> 上面只是示例，请根据自己的项目结构调整引用路径
> 如果要支持 IE 11 请引入 awo-ui/sdk/cxd-ie11.css，但这样就没法支持 CSS 变量了
...
import {render as renderAmis} from 'awo-ui';
import {ToastComponent, AlertComponent, alert, confirm, toast} from 'awo-ui-ui';
      ...
      <div>
        ...
        {renderAmis(
          ...
          },
          {
            ...
            // scopeRef: (ref: any) => (awo-uiScoped = ref)  // 功能和前面 SDK 的 amisScoped 一样
          },
          {
            // 下面三个接口必须实现
            fetcher: ({
...
环境变量，可以理解为这个渲染器工具的配置项，需要使用 awo-ui 用户实现部分接口。他有下面若干参数：
...
可以通过它懒加载自定义组件，比如： https://github.com/baidu/awo-ui/blob/master/__tests__/factory.test.tsx#L64-L91。
