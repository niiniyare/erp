...
func generateRegistryJSON(registry *SchemaRegistry) {
	...
	err = os.WriteFile("schema_registry.json", data, 0644)
	if err != nil {
...
func generateMarkdownReport(registry *SchemaRegistry) {
	...
	err := os.WriteFile("SCHEMA_DISCOVERY_REPORT.md", []byte(md.String()), 0644)
	if err != nil {
...
func generateDependencyGraph(registry *SchemaRegistry) {
	...
	err := os.WriteFile("schema_dependencies.dot", []byte(dot.String()), 0644)
	if err != nil {
